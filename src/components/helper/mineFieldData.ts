...
function validateMockDataRows(dataRows: Array<string>): boolean {
...
export function getMinefieldFromMockData(mockData: string): BoardType {
  const board: BoardType = []

  let preMockBoard = mockData.split('-')
  const mockBoard = preMockBoard.map((row) => {
    return row.split('')
  })
...
export function getNumberOfCellsToUncover(data: BoardType): number {
  let cells = 0
...
export function getNumberOfMines(data: BoardType): number {
  let mines = 0
  for (let row = 0; row < data.length; row += 1) {
    for (let column = 0; column < data[0].length; column += 1) {
...
export function getMinefield(numberOfRows: number, numberOfColumns: number): BoardType {
  const minefieldData: BoardType = []
...
export function minefieldMining(board: BoardType, amount: number): void {
...
export function minefieldNumbering(board: BoardType): void {
  const NUMBER_OF_ROWS = board.length
  const NUMBER_OF_COLUMNS = board[0].length
